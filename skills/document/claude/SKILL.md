...
- Ask the user for input with a clear, concise question (use AskUserQuestion).
...
1. Use Glob to list all source files (`**/*.kt`, `**/*.java`, `**/*.ts`, `**/*.py`, `**/*.go`,
   `**/*.swift`, `**/*.dart`, etc.)
...
- Read the file if a path is given (use Read).
...
directory** using the Write tool. The document must follow this structure:
